...
test("partial-output recovery queues one hidden follow-up", async () => {
	...
	await vault.change((state) => {
		...
		state.settings.policy = "priority-order";
	});
	const quota = {
		fetchedAt: Date.now(),
		primary: { usedPercent: 10, resetAt: Date.now() + 60_000 },
		secondary: { usedPercent: 10, resetAt: Date.now() + 120_000 },
	};
	const first = await vault.add({
		label: "First",
		credential: { access: "a", refresh: "r1", expires: Date.now() + 999_999 },
	});
	const second = await vault.add({
		label: "Second",
		credential: { access: "b", refresh: "r2", expires: Date.now() + 999_999 },
	});
	await vault.update(first.id, (profile) => {
		profile.quota = quota;
		profile.exhaustedUntil = Date.now() + 60_000;
	});
	await vault.update(second.id, (profile) => {
		profile.quota = quota;
	});
	...
	).pending = { requestId: "request", profile: first, failure, queued: false };
	...
	assert.deepEqual(messages[0], {
		message: {
			...
			details: {
				requestId: "request",
				fromProfileId: first.id,
				toProfileId: second.id,
			},
		},
