...
const setup = async () => {
  ...
  return { directory, path: join(directory, "relay", "state.json"), vault: new Vault(join(directory, "relay", "state.json")) };
};
const credential = (access = "access") => ({ access, refresh: "refresh", expires: Date.now() + 60_000 });
...
test("new vault and profile lifecycle", async () => {
  ...
  await vault.update(profile.id, (value) => { value.label = "First"; });
...
test("generation compare-and-swap rejects stale refresh", async () => {
  const { vault } = await setup();
  const profile = await vault.add({ label: "One", credential: credential() });
  assert.equal(await vault.commitRefresh(profile.id, 0, credential("new")), true);
  assert.equal(await vault.commitRefresh(profile.id, 0, credential("stale")), false);
...
test("quarantines corrupt files and enforces modes", async () => {
  ...
  assert.deepEqual(await vault.read(), { version: 1, settings: { policy: "smart-reset", quotaWait: true, priorityOrder: [] }, profiles: {}, migratedNativeAuth: false });
  assert.ok((await readdir(join(directory, "relay"))).some((name) => name.includes(".corrupt-")));
});
