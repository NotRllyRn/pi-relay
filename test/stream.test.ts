...
const profile = (id: string, order: number): RelayProfile => ({
	id,
	provider: "openai-codex",
	label: id,
	credential: { access: `token-${id}`, refresh: "r", expires: 9999999999999 },
	generation: 0,
	enabled: true,
	order,
	createdAt: 0,
...
const events = (...values: AssistantMessageEvent[]) => {
	const stream = createAssistantMessageEventStream();
	queueMicrotask(() => {
		values.forEach((event) => stream.push(event));
		stream.end();
	});
	return stream;
};
...
const dependencies = (
	stream: RelayStreamDependencies["stream"],
	...
	const deps: RelayStreamDependencies = {
		...
		settings: async () => ({
			...
			priorityOrder: profiles.map((value) => value.id),
		}),
		overrides: () => ({}),
		prepare: async (value) => value.credential,
		stream: (model, context, options, value) => {
			attempts.push(value.id);
			return stream(model, context, options, value);
		},
		failure: async (_profile, failure) => {
			failures.push(failure);
		},
		success: async () => undefined,
		continuation: (details) => {
			continuations.push(details);
		},
	};
	return { deps, failures, continuations, attempts };
};
const collect = async (deps: RelayStreamDependencies) => {
	const result: AssistantMessageEvent[] = [];
	for await (const event of streamRelay(model, { messages: [] }, {}, deps))
		result.push(event);
	return result;
};
...
test("retries pre-output quota once with one outer start", async () => {
	const fixture = dependencies((_model, _context, _options, value) =>
		value.id === "a" ? events(start, quota) : events(start, done),
	);
