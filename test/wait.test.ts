...
const profile = (id: string, at: number): RelayProfile => ({ id, provider: "openai-codex", label: id, credential: { access: "a", refresh: "r", expires: 1 }, generation: 0, enabled: true, order: 0, createdAt: 0, updatedAt: 0, exhaustedUntil: at });

test("chooses earliest future reset", () => {
  assert.equal(earliestWake([profile("a", 200), profile("b", 150)], 100)?.profile.id, "b");
});

test("wait wakes once without polling", async () => {
  let wakes = 0;
  const controller = new WaitController(async () => { wakes++; }, undefined, 0);
