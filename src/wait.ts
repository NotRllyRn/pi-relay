...
export const earliestWake = (profiles: RelayProfile[], now = Date.now()): { profile: RelayProfile; at: number } | undefined => {
  const candidates = profiles.filter((profile) => profile.enabled && !profile.needsLogin).flatMap((profile) =>
    [profile.exhaustedUntil, profile.cooldownUntil, profile.quota?.primary?.resetAt, profile.quota?.secondary?.resetAt]
      .filter((at): at is number => at !== undefined && at > now).map((at) => ({ profile, at })),
  );
...
export class WaitController {
  ...
  constructor(
    ...
    private readonly changed: (state: "idle" | "waiting" | "paused", wake?: { profile: RelayProfile; at: number }) => void = () => undefined,
