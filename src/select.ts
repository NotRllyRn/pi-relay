...
export function selectProfile(
	...
	const eligible = profiles.filter((profile) =>
		isEligible(profile, now, attempted),
	);
	const pinned = profiles.find(
		(profile) => profile.id === overrides.pinnedProfileId,
	);
	if (pinned && eligible.includes(pinned))
		return { profile: pinned, source: "pin" };
	const prioritized = profiles.find(
		(profile) => profile.id === overrides.prioritizedProfileId,
	);
	if (prioritized && eligible.includes(prioritized))
		return {
			profile: prioritized,
			source: "priority",
	...
	const ordered = [...eligible].sort(
		policy === "smart-reset"
			? smartComparator
			: policy === "most-available"
				? availableComparator
				: priorityComparator(priorityOrder),
	);
	return {
...
export const earliestFutureReset = (
	profile: RelayProfile,
	now = Date.now(),
): number | undefined => {
	const values = [
...
const permanentlyIneligible = (profile: RelayProfile) =>
	...
	!!profile.needsLogin ||
	limitingRemaining(profile.quota) === 0;
const known = (profile: RelayProfile) =>
	limitingRemaining(profile.quota) === undefined ? 1 : 0;
const limitingReset = (profile: RelayProfile) => {
	const primary = remaining(profile.quota?.primary),
		secondary = remaining(profile.quota?.secondary);
	if (primary === undefined && secondary === undefined)
		return Number.MAX_SAFE_INTEGER;
	return (
		(secondary === undefined || (primary !== undefined && primary <= secondary)
			? profile.quota?.primary?.resetAt
			: profile.quota?.secondary?.resetAt) ?? Number.MAX_SAFE_INTEGER
	);
};
...
const smartComparator = (a: RelayProfile, b: RelayProfile) =>
	compare(
		[
			known(a),
...
const availableComparator = (a: RelayProfile, b: RelayProfile) =>
	compare(
		[
			known(a),
			-(limitingRemaining(a.quota) ?? -1),
			-(remaining(a.quota?.primary) ?? -1),
			limitingReset(a),
			a.lastUsedAt ?? 0,
			a.order,
...
const priorityComparator =
	(order: string[]) => (a: RelayProfile, b: RelayProfile) =>
