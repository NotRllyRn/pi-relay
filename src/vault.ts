...
export class Vault {
  ...
  constructor(readonly path: string) { this.lockPath = `${path}.lock`; }
  ...
  async listProfiles(): Promise<RelayProfile[]> { return Object.values((await this.read()).profiles); }
  async getProfile(id: string): Promise<RelayProfile | undefined> { return (await this.read()).profiles[id]; }

  async add(input: Pick<RelayProfile, "label" | "credential">): Promise<RelayProfile> {
    return this.change((state) => {
      const now = Date.now();
      const profile: RelayProfile = {
        id: randomUUID(), provider: "openai-codex", label: input.label, credential: input.credential,
        generation: 0, enabled: true, order: Object.keys(state.profiles).length, createdAt: now, updatedAt: now,
      };
  ...
  async update(id: string, fn: (profile: RelayProfile) => void): Promise<RelayProfile | undefined> {
    return this.change((state) => {
  ...
  async remove(id: string): Promise<boolean> {
    return this.change((state) => {
      ...
      state.settings.priorityOrder = state.settings.priorityOrder.filter((value) => value !== id);
      return true;
    });
  }

  async commitRefresh(id: string, generation: number, credential: RelayProfile["credential"]): Promise<boolean> {
    return this.change((state) => {
      const profile = state.profiles[id];
  ...
  private async lock<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try { return await fn(); } finally { await rm(this.lockPath, { recursive: true, force: true }); }
  }
...
const isState = (value: unknown): value is RelayState => {
  ...
  return state.version === 1 && !!state.settings && !!state.profiles && typeof state.profiles === "object";
};
