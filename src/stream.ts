...
export function streamRelay(
	...
	context: Context,
	options: SimpleStreamOptions = {},
	dependencies: RelayStreamDependencies,
): AssistantMessageEventStream {
	const outer = createAssistantMessageEventStream();
	const state = { started: false };
	void run(model, context, options, dependencies, outer, state).catch(
		(error) => {
			const aborted = options.signal?.aborted ?? false;
			if (!state.started)
				outer.push({ type: "start", partial: emptyMessage(model, "pending") });
			outer.push({
				type: "error",
				reason: aborted ? "aborted" : "error",
				error: errorMessage(model, error, aborted),
			});
			outer.end();
		},
	);
...
async function run(
	...
	let meaningful = false,
		previous: RelayProfile | undefined,
		lastError: AssistantMessage | undefined;
	for (let pass = 1; pass <= 3; pass++) {
		for (;;) {
			...
			if (!profile) {
				if (
					dependencies.wait &&
					(await dependencies.wait(profiles, options.signal))
				) {
					attempted.clear();
					continue;
				}
			...
			try {
				credential = await dependencies.prepare(profile, options.signal);
			} catch (error) {
			...
			previous = profile;
			const inner = dependencies.stream(
				model,
				context,
			...
			for await (const event of inner) {
				...
				if (event.type === "error") {
					...
					const failure = classifyFailure(
						event.error.errorMessage ?? "Unknown provider error",
					);
					if (options.signal?.aborted || failure.kind === "aborted") {
						outer.push(event);
						outer.end();
						return;
					}
					...
					await dependencies.failure(profile, failure);
					if (switchable && !meaningful) {
						retry = true;
						break;
					}
					if (switchable && meaningful)
						dependencies.continuation?.({ requestId, profile, failure });
					outer.push(event);
					outer.end();
					return;
				}
				outer.push(event);
				if (event.type === "done") {
					await dependencies.success(profile);
	...
	const terminal =
		lastError ?? errorMessage(model, "No eligible Codex account");
	if (!state.started)
		outer.push({ type: "start", partial: emptyMessage(model, "pending") });
	outer.push({
		type: "error",
		reason: terminal.stopReason === "aborted" ? "aborted" : "error",
		error: terminal,
	});
	outer.end();
}
