...
export type QuotaWindow = { usedPercent?: number; resetAt?: number; windowSeconds?: number };
...
export type RelayProfile = {
  ...
  label: string;
  credential: { access: string; refresh: string; expires: number; accountId?: string };
  generation: number;
  enabled: boolean;
  order: number;
  createdAt: number;
...
export type Failure =
  ...
  | { kind: "transient" | "request" | "model" | "aborted" | "unknown"; message: string };
