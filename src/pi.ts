...
export class RelayController {
	...
	private constructor(
		...
		this.wait = new WaitController(
			(profile) => this.refreshProfile(profile, true),
			(state, wake) => this.waitChanged(state, wake),
		);
	}
			...
			auth: {
				apiKey: {
					...
					login: async (interaction) => {
						const label = await interaction.prompt({
							type: "text",
						...
						const refresh = await interaction.prompt({
							type: "secret",
					...
					check: async () =>
						(await this.hasUsableProfile())
							? { type: "api_key", source: "Pi Relay" }
							: undefined,
					resolve: async () =>
						(await this.hasUsableProfile())
							? { auth: { apiKey: MARKER }, source: "Pi Relay" }
							: undefined,
				},
			},
			streamSimple: (model, context, options) =>
				streamRelay(model, context, options, dependencies),
			stream: <T extends "openai-codex-responses">(
				model: Model<T>,
				context: Context,
				...
				streamRelay(
					...
					{
						...dependencies,
						stream: (_model, _context, relayOptions, _profile) =>
							this.base.stream(model, context, {
								...options,
								apiKey: relayOptions.apiKey ?? "",
							} as ApiStreamOptions<T>),
					},
				),
		};
	}
	...
	async restore(context: ExtensionContext): Promise<void> {
		...
		const entry = [...entries]
			...
			.find(
				(value) =>
					value.type === "custom" && value.customType === "pi-relay-state",
			);
		const state =
			entry?.type === "custom" ? (entry.data as SessionRelayState) : undefined;
	...
	async handleAgentEnd(): Promise<void> {
		...
		void this.wait.wait(profiles).then(async (woke) => {
			...
			const selected = selectProfile(
				...
				Date.now(),
				new Set(),
				current.settings.priorityOrder,
			).profile;
	...
	settled(): void {
		if (
			!this.pending?.queued &&
			this.restoredCheckpoint?.phase !== "continuing"
		)
			return;
		...
		if (this.switches)
			this.context?.ui.notify(
				`Relay completed: ${this.switches} switch${this.switches === 1 ? "" : "es"}`,
				"info",
			);
		this.switches = 0;
	}
	...
	async addProfile(
		...
		if (!label.trim() || !access || !refresh)
			throw new Error("Label and both tokens are required");
		...
		const duplicate = duplicateProfile(
			await this.vault.listProfiles(),
			credential,
		);
		if (duplicate)
			throw new Error(`Account already exists as ${duplicate.label}`);
		...
		try {
			...
			if (!(error instanceof UsageError && error.kind === "transient")) {
	...
	async refreshProfile(
		...
		const updated = await this.vault.update(profile.id, (value) => {
			...
			if (limitingRemaining(quota) === 0)
				value.exhaustedUntil =
					earliestFutureReset(value) ?? Date.now() + 60_000;
	...
	async freshProfiles(): Promise<RelayProfile[]> {
		const profiles = (await this.vault.listProfiles()).filter(
			(profile) => profile.enabled && !profile.needsLogin,
		);
		await Promise.allSettled(
			profiles.map((profile) => this.refreshProfile(profile)),
		);
	...
	overrides() {
		return {
			...(this.pinnedProfileId
				? { pinnedProfileId: this.pinnedProfileId }
				: {}),
	...
	overrideWait(profile: RelayProfile): void {
		...
		if (
			...
			this.restoredCheckpoint?.phase === "waiting" ||
			this.restoredCheckpoint?.phase === "continuing"
		) {
			this.wait.cancel();
	...
	private dependencies(): RelayStreamDependencies {
		return {
			...
			settings: async () => {
				const settings = (await this.vault.read()).settings;
				return {
			...
			prepare: (profile, signal) =>
				ensureValidToken(this.vault, profile, signal),
			...
			selected: (profile, previous, selection) =>
				this.recordSelection(profile, previous, selection),
			continuation: (details) => {
				this.pending = { ...details, queued: false };
			},
			wait: async (profiles, signal) =>
				(await this.vault.read()).settings.quotaWait &&
				this.wait.wait(profiles, signal),
		};
	}

	private async recordSelection(
		profile: RelayProfile,
		previous: RelayProfile | undefined,
		selection: Selection,
	): Promise<void> {
		...
		if (previous && previous.id !== profile.id) {
			this.switches++;
			this.context?.ui.notify(
				`Relay: ${previous.label} -> ${profile.label}`,
				"info",
			);
		}
		this.updateStatus(profile);
		await this.log.write("select", {
			profile: fingerprint(profile.id),
	...
	private async hasUsableProfile(): Promise<boolean> {
		return (await this.vault.listProfiles()).some(
			(profile) => profile.enabled && !profile.needsLogin,
		);
	}
	...
	private async migrate(): Promise<void> {
		...
		const credential: Credential | undefined =
			readStoredCredential("openai-codex");
		...
		await this.vault.change((value) => {
			value.migratedNativeAuth = true;
		});
	}
		...
		this.pi.sendMessage(
			{
				...
				display: false,
				details: { requestId, toProfileId: profile.id },
			},
		...
		this.pi.sendMessage(
			{
				...
				details: {
					...
					fromProfileId: pending.profile.id,
					toProfileId: next.id,
				},
			},
			{ triggerTurn: true, deliverAs: "followUp" },
		);
		void this.log.write("continuation-queued", {
			request: pending.requestId,
			profile: fingerprint(next.id),
		});
	}

	private checkpoint(
		phase: SessionRelayState["phase"],
	...
	private waitChanged(
		...
		if (state === "waiting" && wake) {
			...
			this.context?.ui.setStatus(
				"pi-relay",
				`Relay: QUOTA WAIT | ${wake.profile.label} ${formatRelative(wake.at)}`,
			);
...
export const relayPaths = (agentDir: string) => ({
	state: `${agentDir}/pi-relay/state.json`,
	log: `${agentDir}/pi-relay/pi-relay.log`,
});
