...
export class UsageError extends Error {
	constructor(
		readonly kind: "auth" | "rate-limit" | "transient" | "invalid-response",
		message: string,
	) {
		super(message);
	}
...
export async function fetchUsage(
	...
	if (response.status === 401 || response.status === 403)
		throw new UsageError("auth", `Codex usage returned ${response.status}`);
	if (response.status === 429)
		throw new UsageError("rate-limit", "Codex usage is rate limited");
	if (response.status >= 500)
		throw new UsageError(
			"transient",
			`Codex usage returned ${response.status}`,
		);
	if (!response.ok)
		throw new UsageError(
			"invalid-response",
	...
	if (!body || typeof body !== "object")
		throw new UsageError("invalid-response", "Invalid Codex usage response");
	const value = body as Record<string, unknown>;
	const rateLimit = record(value.rate_limit) ?? value;
	const primary = parseWindow(
		rateLimit.primary_window ?? value.primary ?? rateLimit,
		now,
	);
	const secondary = parseWindow(
		rateLimit.secondary_window ?? value.secondary_rate_limit ?? value.secondary,
		now,
	);
	if (!primary && !secondary)
		throw new UsageError("invalid-response", "Codex usage windows are missing");
	return {
		...(primary ? { primary } : {}),
		...(secondary ? { secondary } : {}),
		fetchedAt: now,
...
const parseWindow = (input: unknown, now: number): QuotaWindow | undefined => {
	...
	const resetAt = resetTime(
		value.reset_at ?? window.reset_at,
		value.reset_after_seconds ?? window.reset_after_seconds,
		now,
	);
	const windowSeconds = number(
		value.limit_window_seconds ?? window.limit_window_seconds,
	);
	if (
...
const resetTime = (
	...
	if (typeof reset === "string" && !Number.isFinite(Number(reset))) {
...
export const remaining = (window?: QuotaWindow): number | undefined =>
	window?.usedPercent === undefined
		? undefined
		: Math.max(0, 100 - window.usedPercent);
export const limitingRemaining = (
...
export const isFresh = (
	...
	!!quota &&
	now - quota.fetchedAt < USAGE_TTL_MS &&
	![quota.primary?.resetAt, quota.secondary?.resetAt].some(
		(reset) => reset !== undefined && reset <= now,
	);
...
export const poolUsage = (profiles: RelayProfile[], now = Date.now()) => {
	...
	const primary = average(
		measured.map((profile) => remaining(profile.quota?.primary)),
	);
	const secondary = average(
		measured.map((profile) => remaining(profile.quota?.secondary)),
	);
	return {
		primary,
		secondary,
		effective: minDefined(primary, secondary),
